...
class FForm extends StatefulWidget {
  ...
  /// See also:
  ///
  ///  * [PopScope], another widget that provides a way to intercept the
  ///    back button.
...
class FFormState extends State<FForm> {
  ...
  void _fieldDidChange() {
    ...
    _hasInteractedByUser = _fields.any((FFormFieldState<dynamic> field) => field._hasInteractedByUser);
    _forceRebuild();
  }
  ...
  // bool validate({Set<FFormFieldState<dynamic>> fields = null}) {
  //   _hasInteractedByUser = true;
  //   _forceRebuild();
  //   return _validate(fields: fields);
  // }
  //
  // bool _validate({Set<FFormFieldState<dynamic>> fields = null}) {
  //   var hasError = false;
...
class FFormFieldState<T> extends State<FFormField<T>> {
  ...
  ///
  /// This will not set [errorText] or [hasErrorUpcoming] and it will not update
  /// error display.
  ///
  /// See also:
  ///
  ///  * [validate], which may update [errorText] and [hasErrorUpcoming].
  ...
  void reset() {
    setState(() {
      _value = widget.initialValue as T;
      _hasInteractedByUser = false;
      // _errorText = null;
      // _successText = null;
      _statusText = null;
...
class FTextFieldStatus {
  ...
  FTextFieldStatus({required this.message, this.status = TFStatus.normal});
}
