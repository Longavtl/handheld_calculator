...
class FCupertinoDatePicker extends StatefulWidget {
  ...
  FCupertinoDatePicker({
    ...
    assert(
      mode != FCupertinoDatePickerMode.dateAndTime ||
          minimumDate == null ||
          !this.initialDateTime.isBefore(minimumDate!),
      'initial date is before minimum date',
    );
    assert(
      mode != FCupertinoDatePickerMode.dateAndTime || maximumDate == null || !this.initialDateTime.isAfter(maximumDate!),
      'initial date is after maximum date',
    );
    assert(
      mode != FCupertinoDatePickerMode.date || (minimumYear >= 1 && this.initialDateTime.year >= minimumYear),
      'initial year is not greater than minimum year, or minimum year is not positive',
    );
    assert(
      mode != FCupertinoDatePickerMode.date || maximumYear == null || this.initialDateTime.year <= maximumYear!,
      'initial year is not smaller than maximum year',
    );
    assert(
      mode != FCupertinoDatePickerMode.date || minimumDate == null || !minimumDate!.isAfter(this.initialDateTime),
      'initial date ${this.initialDateTime} is not greater than or equal to minimumDate $minimumDate',
    );
    assert(
      mode != FCupertinoDatePickerMode.date || maximumDate == null || !maximumDate!.isBefore(this.initialDateTime),
...
class _CupertinoDatePickerDateTimeState extends State<FCupertinoDatePicker> {
  ...
  Widget build(BuildContext context) {
    ...
    final columnWidths = <double>[
      _getEstimatedColumnWidth(_PickerColumnType.hour)??12.0,
      _getEstimatedColumnWidth(_PickerColumnType.minute)??0.0,
    ];
    ...
    if (!widget.use24hFormat) {
      if (localizations!.datePickerDateTimeOrder == DatePickerDateTimeOrder.date_time_dayPeriod ||
        ...
        columnWidths.add(_getEstimatedColumnWidth(_PickerColumnType.dayPeriod)??10.0);
      } else {
        pickerBuilders.insert(0, _buildAmPmPicker);
        columnWidths.insert(0, _getEstimatedColumnWidth(_PickerColumnType.dayPeriod)??10.0);
      }
    }
    ...
    if (widget.mode == FCupertinoDatePickerMode.dateAndTime) {
      if (localizations!.datePickerDateTimeOrder == DatePickerDateTimeOrder.time_dayPeriod_date ||
        ...
        columnWidths.add(_getEstimatedColumnWidth(_PickerColumnType.date)??10.0);
      } else {
        pickerBuilders.insert(0, _buildMediumDatePicker);
        columnWidths.insert(0, _getEstimatedColumnWidth(_PickerColumnType.date)??10.0);
      }
    }
    ...
    return NotificationListener<ScrollNotification>(
      ...
      child: CupertinoPicker(
        ...
        children: List<Widget>.generate(12, (int index) {
          final month = index + 1;
          final isInvalidMonth = (widget.minimumDate?.year == selectedYear && widget.minimumDate!.month > month) ||
    ...
    return NotificationListener<ScrollNotification>(
      ...
      child: CupertinoPicker.builder(
        ...
        itemBuilder: (BuildContext context, int year) {
          ...
          final isValidYear = (widget.minimumDate == null || widget.minimumDate!.year <= year) &&
