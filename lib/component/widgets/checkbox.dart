...
class _FCheckboxState extends State<FCheckbox> with TickerProviderStateMixin, ToggleableStateMixin {
  ...
  @override
  ValueChanged<bool?>? get onChanged => (bool? value) {
  ...
  MaterialStateProperty<Color> get _widgetFillColor {
    return MaterialStateProperty.resolveWith((Set<MaterialState> states) {
      if (states.contains(MaterialState.disabled)) {
        return Colors.grey; // Trả về màu xám khi disabled
      }
      if (states.contains(MaterialState.selected)) {
        return widget.activeColor ?? Colors.blue; // Trả về màu xanh khi selected
      }
      return Colors.white; // Trả về màu trắng mặc định
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    switch (effectiveMaterialTapTargetSize) {
      ...
      case MaterialTapTargetSize.shrinkWrap:
        size = Size(kTapSize - widget.size.size / 2, kTapSize - widget.size.size / 2);
        break;
    }
    size += effectiveVisualDensity.baseSizeAdjustment;

    final effectiveMouseCursor = MaterialStateProperty.resolveWith<MouseCursor>((Set<MaterialState> states) {
    ...
    return Semantics(
      ...
      child: buildToggleable(
        ...
        painter: _painter
          ...
          ..side = BorderSide(width: widget.size.strokeWidth, color: widget.borderColor ?? FColors.grey4)
...
class _CheckboxPainter extends ToggleablePainter {
  ...
  Color get checkColor => _checkColor ?? Colors.black; // Trả về màu đen khi null
  Color ?_checkColor;
  ...
  bool get previousValue => _previousValue??false;
  bool ?_previousValue;
  ...
  OutlinedBorder get shape => _shape ?? RoundedRectangleBorder();// Trả về OutlinedBorder mặc định khi null
  ...
  BorderSide get side => _side?? BorderSide();
  BorderSide ?_side;
  ...
  void _drawCheck(Canvas canvas, Offset origin, double t, Paint paint) {
    ...
    final path = Path();
    final start = Offset(sizeValue!.size * 0.28, sizeValue!.size * 0.48);
    final mid = Offset(sizeValue!.size * 0.45, sizeValue!.size * 0.65);
    final end = Offset(sizeValue!.size * 0.75, sizeValue!.size * 0.3);
    if (t < 0.5) {
      final strokeT = t * 2.0;
  ...
  void _drawDash(Canvas canvas, Offset origin, double t, Paint paint) {
    ...
    // mid point outwards.
    var start = Offset(sizeValue!.size * 0.2, sizeValue!.size * 0.5);
    var mid = Offset(sizeValue!.size * 0.5, sizeValue!.size * 0.5);
    var end = Offset(sizeValue!.size * 0.8, sizeValue!.size * 0.5);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final origin = size / 2.0 - Size.square(sizeValue!.size) / 2.0 as Offset;
    ...
    if (previousValue == false || value == false) {
      ...
      final emptyCheckboxPath = shape.copyWith(side: side).getOuterPath(outer);
